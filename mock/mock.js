...
// 左侧菜单config
app.all('/cdp-web/sys/config', function (req, res) {
    const config = require('./config.json');
    res.send(config);
});
app.all('/cdp-web/user/session', function (req, res) {
    const channel = require('./session.json');
    res.send(channel);
});
app.all('/cdp-web/types/cache', function (req, res) {
    const cache = require('./cache.json');
    res.send(cache);
});

// 左侧菜单项
app.all('/cdp-web/user/channel', function (req, res) {
...
// 获取全部站内信
app.get('/cdp-web/sys/messages/list/all', function (req, res) {
    const messageList = require('./sysMessage/messageList.json');
    res.send(messageList);
});

// 获取未读站内信数
app.get('/cdp-web/sys/messages/count/unread', function (req, res) {
    res.json(4);
});

// 将未读站内信设置为已读
app.put('/cdp-web/sys/messages/read/message-list', function (req, res) {
    res.json();
});

// 渠道管理员-运营架构树
app.get('/cdp-web/sys/orgs/list-orgs', function (req, res) {
    const listOrgs = require('./manage/operationTree.json');
    res.send(listOrgs);
});

// 渠道管理员-运营架构树根节点
app.get('/cdp-web/sys/orgs/list-root-orgs', function (req, res) {
    const listRootOrgs = require('./manage/operationTreeRoot.json');
    res.send(listRootOrgs);
});

// 渠道管理员-修改节点信息
app.put('/cdp-web/sys/orgs/*', function (req, res) {
    res.json();
});

// 渠道管理员-运营资源-资源转移
app.put('/cdp-web/sys/resource/*', function (req, res) {
    const transResource = require('./manage/transResource.json');
    res.send(transResource);
});

// 渠道管理员-运营资源-资源转移-原所属人
app.get('/cdp-web/sys/resource/list-original-operator?*', function (req, res) {
    const oriList = require('./manage/oriList.json');
    res.send(oriList);
});

// 渠道管理员-运营资源-资源转移-目标所属人
app.get('/cdp-web/sys/resource/list-target-operator?*', function (req, res) {
    const targetList = require('./manage/targetList.json');
    res.send(targetList);
});

// 渠道管理员-运营资源-资源转移-原运营组
app.get('/cdp-web/sys/resource/list-original-group?*', function (req, res) {
    const oriGroupList = require('./manage/oriGroupList.json');
    res.send(oriGroupList);
});

// 渠道管理员-运营资源-资源转移-目标运营组
app.get('/cdp-web/sys/resource/list-target-group?*', function (req, res) {
    const targetGroupList = require('./manage/targetGroupList.json');
    res.send(targetGroupList);
});

// 渠道管理员-新增架构节点
app.post('/cdp-web/sys/orgs/dept', function (req, res) {
    res.json('38123189');
});

// 审核-审核控制列表
app.get('/cdp-web/api/audit/query-configs', function (req, res) {
    const auditList = require('./audit/auditControl.json');
    res.send(auditList);
});

// 审核-审核开关控制
app.put('/cdp-web/api/audit/update-config?*', function (req, res) {
    res.json();
});

// 租户管理员-获取渠道数据集
app.get('/cdp-web/sys/channels/', function (req, res) {
    const channelList = require('./manage/channelList.json');
    res.send(channelList);
});

// 租户管理员-取消授权渠道管理员
app.delete('/cdp-web/sys/auth/channels/*', function (req, res) {
    res.json();
});

// 租户管理员-修改渠道名称
app.put('/cdp-web/sys/channels/*', function (req, res) {
    res.json();
});

// 租户管理员-查询已授权管理员
app.get('/cdp-web/sys/auth/channels/*', function (req, res) {
    const managerList = require('./manage/getAuthManager.json');
    res.send(managerList);
});

// 租户管理员-查询需要被授权的渠道管理员
app.get('/cdp-web/sys/auth/channels/*/users', function (req, res) {
    const needManagerList = require('./manage/getNeedManager.json');
    res.send(needManagerList);
});

// 租户管理员-新增授权的渠道管理员
app.post('/cdp-web/sys/auth/channels/*', function (req, res) {
    res.json();
});

// 租户管理员-查询渠道类型
app.get('/cdp-web/sys/channels/type', function (req, res) {
    const channelType = require('./manage/channelType.json');
    res.send(channelType);
});

// 租户管理员-增加渠道
app.post('/cdp-web/sys/channels', function (req, res) {
    res.json();
});

// 渠道管理员-获取公共特性
app.get('/cdp-web/traits?size=-1&type_code=STRING&type_code=STRING_SET&visibility=public', function (req, res) {
    const traitsConfigList = require('./manage/traitsConfigList.json');
    res.send(traitsConfigList);
});

// 数据-特性管理列表
app.get('/cdp-web/traits', function (req, res) {
...
// 用户-人群管理列表
app.get('/cdp-web/crowds', function (req, res) {
    const list = require('./user/list.json');
    res.send(list);
});

// 用户-人群管理详情
app.get('/cdp-web/crowds/combination/*', function (req, res) {
    const crowd = require('./user/crowd.json');
    res.send(crowd);
});

// 节点管理员-新增运营组
app.post('/cdp-web/sys/groups', function (req, res) {
    res.json();
});

// 节点管理员-修改运营组
app.put('/cdp-web/sys/groups', function (req, res) {
    res.json();
});

// 节点管理员-删除运营组
app.delete('/cdp-web/sys/groups/*', function (req, res) {
    res.json();
});

// 节点管理员-查询运营组列表
app.get('/cdp-web/sys/groups', function (req, res) {
    const groupList = require('./group/groupList.json');
    res.send(groupList);
});

// 节点管理员-新增运营组-运营组成员列表
app.get('/cdp-web/sys/groups/org-members', function (req, res) {
