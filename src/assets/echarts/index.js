...
}(this, __WEBPACK_EXTERNAL_MODULE_2__ =>
...
/* 1 */
/** */ function (module, exports, __webpack_require__) {
	const echarts = __webpack_require__(2);
...
/* 3 */
/** */ function (module, exports, __webpack_require__) {
	const completeDimensions = __webpack_require__(4);
	const echarts = __webpack_require__(2);
	...
	echarts.extendSeriesModel({
	    ...
	    getInitialData(option, ecModel) {
	        const dimensions = completeDimensions(['value'], option.data);
...
/** */ function (module, exports, __webpack_require__) {
	 ...
	 */
	    ...
	    const zrUtil = __webpack_require__(5);
	    ...
	    function completeDimensions(dimensions, data, defaultNames, extraPrefix) {
	        ...
	        const value0 = retrieveValue(data[0]);
	        const dimSize = zrUtil.isArray(value0) && value0.length || 1;

	        defaultNames = defaultNames || [];
	        extraPrefix = extraPrefix || 'extra';
	        for (let i = 0; i < dimSize; i++) {
	            if (!dimensions[i]) {
	                const name = defaultNames[i] || (extraPrefix + (i - defaultNames.length));
	    ...
	    var guessOrdinal = completeDimensions.guessOrdinal = function (data, dimIndex) {
	        for (let i = 0, len = data.length; i < len; i++) {
	            ...
	            if (zrUtil.isString(value) && value !== '-') {
	                return true;
	            }
	        }
	        return false;
...
/** */ function (module, exports) {
	 ...
	 */
	    ...
	    function bind(func, context) {
	        const args = nativeSlice.call(arguments, 2);
	        return function () {
...
/** */ function (module, exports, __webpack_require__) {
	...
	const symbolUtil = __webpack_require__(7);
	const parsePercent = numberUtil.parsePercent;

	const LiquidLayout = __webpack_require__(63);
	...
	echarts.extendChartView({
	    ...
	    render(seriesModel, ecModel, api) {
	        ...
	        const outterRadius = parsePercent(radius, size) / 2;
	        const innerRadius = outterRadius - outlineBorderWidth / 2;
	        const paddingRadius = parsePercent(outlineDistance, size);
	        ...
	        data.diff(oldData)
	            .add((idx) => {
	                const wave = getWave(idx, false);

	                const waterLevel = wave.shape.waterLevel;
	            ...
	            .update((newIdx, oldIdx) => {
	                ...
	                const newWave = getWave(newIdx, false, waveElement);
	            ...
	            .remove((idx) => {
	                const wave = oldData.getItemGraphicEl(idx);
	                group.remove(wave);
	            })
	            .execute();
	        ...
	        function getPath(r, isForClipping) {
	            const symbol = seriesModel.get('shape');
	            if (symbol) {
	                // customed symbol path
	                if (symbol.indexOf('path://') === 0) {
	                    let path = echarts.graphic.makePath(symbol.slice(7), {});
	                    const bouding = path.getBoundingRect();
	                    let width = bouding.width;
	                    let height = bouding.height;
	                    if (width > height) {
	                    ...
	                    const left = isForClipping ? 0 : cx - width / 2;
	                    const top = isForClipping ? 0 : cy - height / 2;
	         ...
	         */
	        function getOutline() {
	            const outlinePath = getPath(outterRadius);
	        ...
	        function getBackground() {
	            ...
	            const strokePath = getPath(radius);
	        ...
	        function getWave(idx, isInverse, oldWave) {
	            ...
	            const amplitude = parsePercent(itemModel.get('amplitude'),
	                radius * 2);
	            const waveLength = parsePercent(itemModel.get('waveLength'),
	                radius * 2);

	            const value = data.get('value', idx);
	            const waterLevel = radius - value * radius * 2;
	            phase = oldWave ? oldWave.shape.phase
	                : (phase === 'auto' ? idx * Math.PI / 4 : phase);
	            const normalStyle = itemStyleModel.getModel('normal').getItemStyle();
	            normalStyle.fill = data.getItemVisual(idx, 'color');

	            const x = radius * 2;
	            const wave = new LiquidLayout({
	                shape: {
	                    waveLength,
	                    radius,
	                    cx: x,
	            ...
	            const hoverStyle = itemStyleModel.getModel('emphasis').getItemStyle();
	            hoverStyle.lineWidth = 0;
	            echarts.graphic.setHoverStyle(wave, hoverStyle);

	            // clip out the part outside the circle
	            const clip = getPath(radius, true);
	        ...
	        function setWaveAnimation(idx, wave, oldWave) {
	            ...
	            const direction = itemModel.get('direction');

	            const value = data.get('value', idx);
	            const value0 = data.get('value', 0);
	        ...
	        function getText(waves) {
	            const labelModel = itemModel.getModel('label.normal');
	            const textStyle = labelModel.getModel('textStyle');

	            function formatLabel() {
	                const formatted = seriesModel.getFormattedLabel(0, 'normal');
	                const defaultVal = (data.get('value', 0) * 100);
	            ...
	            const outsideTextRect = new echarts.graphic.Rect(textOption);
	            const color = textStyle.get('color');
	            echarts.graphic.setText(outsideTextRect.style, labelModel, color);

	            const insideTextRect = new echarts.graphic.Rect(textOption);
	            const insColor = textStyle.get('insideColor');
	            ...
	            const boundingCircle = getPath(radius, true);
...
/** */ function (module, exports, __webpack_require__) {
	// Symbol factory
	    ...
	    const graphic = __webpack_require__(8);
	    const BoundingRect = __webpack_require__(28);

	    /**
	     * Triangle shape
	     * @inner
	     */
	    const Triangle = graphic.extendShape({
	        type: 'triangle',
	        shape: {
	            cx: 0,
	            cy: 0,
	     ...
	     * @inner
	     */
	    const Diamond = graphic.extendShape({
	        type: 'diamond',
	        shape: {
	            cx: 0,
	            cy: 0,
	     ...
	     * @inner
	     */
	    const Arrow = graphic.extendShape({
	    ...
	    for (const name in symbolCtors) {
	        if (symbolCtors.hasOwnProperty(name)) {
	            symbolBuildProxies[name] = new symbolCtors[name]();
	        }
	    }

	    const Symbol = graphic.extendShape({
	    ...
	    const symbolPathSetColor = function (color) {
	        if (this.type !== 'image') {
	    ...
	    const symbolUtil = {
	         ...
	         * @param {string} color
	         */
	        createSymbol(symbolType, x, y, w, h, color) {
...
/** */ function (module, exports, __webpack_require__) {
	    ...
	    const pathTool = __webpack_require__(9);
	    const round = Math.round;
	    const Path = __webpack_require__(10);
	    const colorTool = __webpack_require__(23);
	    const matrix = __webpack_require__(17);
	    const vector = __webpack_require__(18);
	    ...
	    graphic.subPixelOptimizeLine = function (param) {
	        const subPixelOptimize = graphic.subPixelOptimize;
	        const shape = param.shape;
	        const lineWidth = param.style.lineWidth;
	    ...
	    graphic.subPixelOptimizeRect = function (param) {
	        ...
	        const originWidth = shape.width;
	        const originHeight = shape.height;
	    ...
	    function cacheElementStl(el) {
	        if (el.__hoverStlDirty) {
	            ...
	            const hoverStyle = el.__hoverStl;
	    ...
	    function doSingleLeaveHover(el) {
	        ...
	        const normalStl = el.__normalStl;
	        if (el.useHoverLayer) {
	    ...
	    graphic.setText = function (textStyle, labelModel, color) {
	        const labelPosition = labelModel.getShallow('position') || 'inside';
	        const labelColor = labelPosition.indexOf('inside') >= 0 ? 'white' : color;
	        const textStyleModel = labelModel.getModel('textStyle');
	    ...
	    function animateOrSetProps(isUpdate, el, props, animatableModel, dataIndex, cb) {
	        ...
	        const animationEnabled = animatableModel
	            && (
	        ...
	        if (animationEnabled) {
	            const postfix = isUpdate ? 'Update' : '';
	            let duration = animatableModel
	                && animatableModel.getShallow(`animationDuration${postfix}`);
	            const animationEasing = animatableModel
	                && animatableModel.getShallow(`animationEasing${postfix}`);
	            let animationDelay = animatableModel
...
/** */ function (module, exports, __webpack_require__) {
	    const Path = __webpack_require__(10);
	    const PathProxy = __webpack_require__(29);
	    const transformPath = __webpack_require__(40);
	    const matrix = __webpack_require__(17);
	    ...
	    function createPathProxyFromString(data) {
	        ...
	        for (n = 1; n < arr.length; n++) {
	            ...
	            let c = str.charAt(0);
	            let off = 0;
	            const p = str.slice(1).replace(/e,-/g, 'e-').split(',');
	            var cmd;
...
/** */ function (module, exports, __webpack_require__) {
	 ...
	 */
	    ...
	    const Displayable = __webpack_require__(11);
	    const zrUtil = __webpack_require__(5);
	    const PathProxy = __webpack_require__(29);
	    const pathContain = __webpack_require__(32);

	    const Pattern = __webpack_require__(39);
	    ...
	    Path.extend = function (defaults) {
	        const Sub = function (opts) {
	            ...
	            const defaultShape = defaults.shape;
	            if (defaultShape) {
	                this.shape = this.shape || {};
	                const thisShape = this.shape;
	                for (const name in defaultShape) {
	                    if (
...
/** */ function (module, exports, __webpack_require__) {
	 ...
	 */
	    ...
	    const Element = __webpack_require__(13);
	    const RectText = __webpack_require__(26);
...
/** */ function (module, exports) {
	 ...
	 */
	    ...
	    Style.prototype = {
	         ...
	         */
	        clone() {
	            const newStyle = new this.constructor();
	            newStyle.extendFrom(this, true);
	            return newStyle;
	        },

	        getGradient(ctx, obj, rect) {
	            const method = obj.type === 'radial' ? createRadialGradient : createLinearGradient;
	            const canvasGradient = method(ctx, obj, rect);
	    ...
	    const styleProto = Style.prototype;
	    for (let i = 0; i < STYLE_COMMON_PROPS.length; i++) {
	        const prop = STYLE_COMMON_PROPS[i];
...
/** */ function (module, exports, __webpack_require__) {
	 ...
	 */
	    ...
	    const guid = __webpack_require__(14);
	    const Eventful = __webpack_require__(15);
	    const Transformable = __webpack_require__(16);
	    const Animatable = __webpack_require__(19);
	    const zrUtil = __webpack_require__(5);
...
/** */ function (module, exports, __webpack_require__) {
	 ...
	 */
	     ...
	     * @constructor
	     */
	    const Transformable = function (opts) {
	        opts = opts || {};
	    ...
	    const transformableProto = Transformable.prototype;
...
/** */ function (module, exports, __webpack_require__) {
	 ...
	 */
	    ...
	    const Animator = __webpack_require__(20);
	    ...
	    Animatable.prototype = {
	        ...
	        animateTo(target, time, delay, easing, callback) {
	            ...
	            const animators = this.animators.slice();
...
/** */ function (module, exports, __webpack_require__) {
	 ...
	 */
	    ...
	    const Clip = __webpack_require__(21);
	    ...
	    function createTrackClip(animator, easing, oneTrackDone, keyframes, propName) {
	        ...
	        for (var i = 0; i < trackLen - 1; i++) {
	            ...
	            else if (isNaN(kfValues[i]) && !isNaN(lastValue) && !isValueString && !isValueColor) {
	     ...
	     */
	    const Animator = function (target, loop, getter, setter) {
	        this._tracks = {};
	        this._target = target;
...
/** */ function (module, exports, __webpack_require__) {
	 ...
	 */
	    ...
	    const easingFuncs = __webpack_require__(22);
	    ...
	    Clip.prototype = {
	        ...
	        step(globalTime) {
	            ...
	            let percent = (globalTime - this._startTime) / this._life;
	            ...
	            const easingFunc = typeof easing == 'string' ? easingFuncs[easing] : easing;
	        ...
	        restart(globalTime) {
	            const remainder = (globalTime - this._startTime) % this._life;
...
/** */ function (module, exports, __webpack_require__) {
	 ...
	 */
	    ...
	    const textContain = __webpack_require__(27);
	    const BoundingRect = __webpack_require__(28);

	    const tmpRect = new BoundingRect();

	    const RectText = function () {};
	    ...
	    RectText.prototype = {
	         ...
	         */
	        drawRectText(ctx, rect, textRect) {
	            const style = this.style;
	            let text = style.text;
	            // Convert to string
	            text != null && (text += '');
	            ...
	            const distance = style.textDistance;
	            let align = style.textAlign;
	            const font = style.textFont || style.font;
	            let baseline = style.textBaseline;
	            const verticalAlign = style.textVerticalAlign;
	            ...
	            if (textPosition instanceof Array) {
	                ...
	                const res = textContain.adjustTextPositionOnRect(
...
/** */ function (module, exports, __webpack_require__) {
	    ...
	    const util = __webpack_require__(5);
	    const BoundingRect = __webpack_require__(28);
	    const retrieve = util.retrieve;

	    function getTextWidth(text, textFont) {
	        const key = `${text}:${textFont}`;
	    ...
	    function getTextRect(text, textFont, textAlign, textBaseline) {
	        const textLineLen = (`${text || ''}`).split('\n').length;

	        const width = getTextWidth(text, textFont);
	        // FIXME 楂樺害璁＄畻姣旇緝绮楁毚
	        const lineHeight = getTextWidth('鍥�', textFont);
	        const height = textLineLen * lineHeight;

	        const rect = new BoundingRect(0, 0, width, height);
	    ...
	    function adjustTextPositionOnRect(textPosition, rect, textRect, distance) {
	        ...
	        const halfHeight = height / 2 - textHeight / 2;
	    ...
	    function truncateText(text, containerWidth, textFont, ellipsis, options) {
	        ...
	        const maxIterations = retrieve(options.maxIterations, 2);
	        const minChar = retrieve(options.minChar, 0);
	        // FIXME
	        // Other languages?
	        const cnCharWidth = getTextWidth('鍥�', textFont);
	        // FIXME
	        // Consider proportional font?
	        const ascCharWidth = getTextWidth('a', textFont);
	        const placeholder = retrieve(options.placeholder, '');
...
/** */ function (module, exports, __webpack_require__) {
	 ...
	 */
	    ...
	    const vec2 = __webpack_require__(18);
	    const matrix = __webpack_require__(17);
...
/** */ function (module, exports, __webpack_require__) {
	 ...
	 */
	 ...
	 // TODO getTotalLength, getPointAtLength
	    ...
	    const curve = __webpack_require__(30);
	    const vec2 = __webpack_require__(18);
	    const bbox = __webpack_require__(31);
	    const BoundingRect = __webpack_require__(28);
	    const dpr = __webpack_require__(25).devicePixelRatio;
...
/* 32 */
/** */ function (module, exports, __webpack_require__) {
	    const CMD = __webpack_require__(29).CMD;
	    const line = __webpack_require__(33);
	    const cubic = __webpack_require__(34);
	    const quadratic = __webpack_require__(35);
	    const arc = __webpack_require__(36);
	    const normalizeRadian = __webpack_require__(37).normalizeRadian;
	    const curve = __webpack_require__(30);

	    const windingLine = __webpack_require__(38);

	    const containStroke = line.containStroke;
...
y1_;
	            for (let i = 0; i < nRoots; i++) {
	                ...
	                const x_ = curve.cubicAt(x0, x1, x2, x3, t);
	    ...
	    function windingQuadratic(x0, y0, x1, y1, x2, y2, x, y) {
	        ...
	        if (nRoots === 0) {
	            ...
	            if (t >= 0 && t <= 1) {
	                let w = 0;
	                const y_ = curve.quadraticAt(y0, y1, y2, t);
	                for (let i = 0; i < nRoots; i++) {
	                    // Remove one endpoint.
...
/** */ function (module, exports, __webpack_require__) {
	    ...
	    module.exports = {
	        ...
	        containStroke(x0, y0, x1, y1, x2, y2, lineWidth, x, y) {
	            ...
	            const d = curve.quadraticProjectPoint(
	                x0, y0, x1, y1, x2, y2,
	                x, y, null
	            );
	            return d <= _l / 2;
...
/* 39 */
/** */ function (module, exports) {
	    const Pattern = function (image, repeat) {
	        this.image = image;
	        this.repeat = repeat;
...
/** */ function (module, exports, __webpack_require__) {
	 ...
	 */
	    ...
	    const LRU = __webpack_require__(43);
	    ...
	    ZImage.prototype = {
	        ...
	        brush(ctx, prevEl) {
	            ...
	            if (image) {
	                ...
	                const width = style.width || image.width;
	                const height = style.height || image.height;
	                const x = style.x || 0;
	                const y = style.y || 0;
	                // 鍥剧墖鍔犺浇澶辫触
	                if (!image.width || !image.height) {
...
/** */ function (module, exports) {
	...
	// @module zrender/core/LRU
	    ...
	    const linkedListProto = LinkedList.prototype;
	    /**
	    ...
	    Text.prototype = {
	        ...
	        brush(ctx, prevEl) {
	            ...
	            if (text) {
	                ...
	                const lineHeight = textContain.measureText('鍥�', ctx.font).width;
...
/** */ function (module, exports, __webpack_require__) {
	 ...
	 */
	    ...
	    const env = __webpack_require__(47);
	    const Path = __webpack_require__(10);
	            ...
	            ? function () {
	                ...
	                if (clipPaths) {
	                    for (let i = 0; i < clipPaths.length; i++) {
	                        const shape = clipPaths[i] && clipPaths[i].shape;
...
/** */ function (module, exports, __webpack_require__) {
	 ...
	 */
	    ...
	    const polyHelper = __webpack_require__(50);
...
/* 50 */
/** */ function (module, exports, __webpack_require__) {
	    const smoothSpline = __webpack_require__(51);
	    const smoothBezier = __webpack_require__(52);
...
/** */ function (module, exports, __webpack_require__) {
	 ...
	 */
	    ...
	    const vec2 = __webpack_require__(18);
	    const v2Min = vec2.min;
	    const v2Max = vec2.max;
	    const v2Scale = vec2.scale;
	    const v2Distance = vec2.distance;
	    const v2Add = vec2.add;
...
/** */ function (module, exports, __webpack_require__) {
	 ...
	 */
	    ...
	    const roundRectHelper = __webpack_require__(55);
...
/** */ function (module, exports, __webpack_require__) {
	 ...
	 */
	    ...
	    const curveTool = __webpack_require__(30);
	    const vec2 = __webpack_require__(18);
	    ...
	    const quadraticDerivativeAt = curveTool.quadraticDerivativeAt;
	    const cubicDerivativeAt = curveTool.cubicDerivativeAt;
...
/** */ function (module, exports, __webpack_require__) {
	    ...
	    const Gradient = __webpack_require__(61);
...
/** */ function (module, exports, __webpack_require__) {
	     ...
	     * @param {boolean} [globalCoord=false]
	     */
	    const RadialGradient = function (x, y, r, colorStops, globalCoord) {
	        this.x = x == null ? 0.5 : x;
...
/** */ function (module, exports, __webpack_require__) {
	...
	module.exports = echarts.graphic.extendShape({
	    ...
	    buildPath(ctx, shape) {
	        const curves = Math.ceil(2 * shape.radius / shape.waveLength * 4) * 2;
	        const controls = [[0, 0]];
	        const positions = [];
	        ...
	        const phase = shape.phase / Math.PI / 2 * shape.waveLength;

	        const left = shape.cx - shape.radius + phase - shape.radius * 2;
	        ...
	        for (let c = 0; c < curves; ++c) {
	            const stage = c % 4;
	            const pos = getWaterPositions(c * shape.waveLength / 4, stage,
...
/** */ function (module, exports) {
	// Pick color from palette for each data item
	    ...
	    module.exports = function (seriesType, ecModel) {
	        ...
	        const paletteScope = {};
	        ecModel.eachRawSeriesByType(seriesType, (seriesModel) => {
	            ...
	            if (!ecModel.isSeriesFiltered(seriesModel)) {
	                ...
	                dataAll.each((rawIdx) => {
	                    // FIXME Performance
	                    const itemModel = dataAll.getItemModel(rawIdx);
	                    ...
	                    if (!singleDataColor) {
	                        const color = itemModel.get('itemStyle.normal.color')
