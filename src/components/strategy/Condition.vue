<template>
    <div>
        <div class="and-box" v-if="conditions.length">
            ...
            <div class="and-condition">
                <Row :class="{'condition-row-no-padding': conditions.length==1}" type="flex" align="middle" justify="start" v-for="(item,index) in conditions" :key="item.id" class="add-condition-row">
                    ...
                    <Col v-role-button="`strategy_trait_button_${$route.query.tabView}`" v-if="!isReadOnly" class="add-condition-del-cell">
        ...
        <Button :class="{'pr60':conditions.length>1}" v-role-button="`strategy_trait_button_${$route.query.tabView}`" v-if="!isReadOnly" icon="md-add" type="primary" ghost @click="addCondition" style="margin-top:10px;">添加条件</Button>
    </div>
</template>
