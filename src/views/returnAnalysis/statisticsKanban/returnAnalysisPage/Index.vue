  ...
  <div>
    ...
    <div v-else>
      <Row
        ...
        align="middle"
        style="padding: 16px 24px; background-color: white; margin-top: 15px"
      >
        <i-col style="background-color: white">
          ...
          <span style="font-size: 20px">
            【 <span style="color: #ff905a; font-weight: 600">{{ code }}</span> 】
          </span>
        </i-col>
      </Row>
          ...
          <i-col style="display: flex">
            ...
            <!-- <i-col>
              ...
              <Tooltip
                ...
                placement="right"
              >
                <Button :disabled="disabled" type="primary" @click="inquire"
                  >查询</Button
                >
              </Tooltip>
            </i-col> -->
...
<script>
    ...
    export default {
        ...
        created() {
            this.code = [sessionStorage.getItem('sourceType'), sessionStorage.getItem('startTimeDay'), sessionStorage.getItem('endTimeDay')].toString();
            // this.$https.analysisManagement.allQueryType().then((res) => {
            //     this.ditchList = res[0].data.data.map(item => ({
            //         label: item.name,
            //         value: item.orgId
            //     }));
            //     this.ditchList.unshift({ label: '选择渠道', value: '0' });
            //     this.areaList = res[1].data.data.map(item => ({
            //         label: item.name,
            //         value: item.orgId
            //     }));
        ...
        methods: {
            ...
            ditchChange(e) {
                if (e !== '0') {
                    this.queryOptions.brand = this.find(this.ditchList, 'value', e)
                        && (this.find(this.ditchList, 'value', e).label || '');
                } else {
            ...
            areaChange(e) {
                if (e !== '0') {
                    this.queryOptions.area = this.find(this.areaList, 'value', e)
                        && (this.find(this.areaList, 'value', e).label || '');
                } else {
            ...
            shopChange(e) {
                ...
                for (const i in e) {
                    arr.push(
                        this.find(this.shopList, 'value', e[i])
                            && (this.find(this.shopList, 'value', e[i]).label || '')
                    );
                }
                if (!arr.length) {
                    this.disabled = true;
