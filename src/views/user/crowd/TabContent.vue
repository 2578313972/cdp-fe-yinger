    ...
    <div class="crowd-manage-box">
        <div class="h100 crowd-manage-left">
            <div class="crowd-manage-left-Tree">
                <custom-tree
                    v-role-button="`catalog_tree_crowd_${tabView}`"
                    v-if="!treeLoading"
                    ref="trait_tree"
                    :data="iniTreeList"
                ...
                <Card dis-hover :padding="0">
                    <Row type="flex" justify="space-between" style="margin:16px">
                        <Col>
                            ...
                            <Input class="width300 ml10"  @on-change="debounceSearch" v-model="crowdName" placeholder="请输入人群名称/ID搜索">
                                <Icon slot="suffix" type="ios-search"/>
                            </Input>
                            <span class="total-count">共<i>{{page.total}}</i>个人群</span>
                        </Col>
                        <Col>
                            <Button
                                icon="md-add"
                                v-role-button="'create_crowd_button_' + tabView"
                                type="primary"
                                v-if="iniTreeList.length"
                                @click="f_add=true">新建人群
                            </Button>
...
<script>
    import AddCrowd from '@/views/user/crowd/Add';
    ...
    export default {
        ...
        data() {
            return {
                trait_crowd: 'crowd',
                treeLoading: false, // tree的loading,用于加载运营组不显示tree的”暂无数据“字样。
                f_add: false, // 新建人群
                ...
                groupID: '', // 组id
                groupName: '', // 组name
                childTreeList: [], // 更新自己的tree data
                loading: true, // table list 的loading
                crowdName: '', // 搜索人群名称
                order: '', // desc  asc
                sortName: '', // 排序name
                listData: [],
        ...
        methods: {
            ...
            getList() {
                ...
                const get_params = {
                    ...
                    q: this.crowdName || undefined, // 搜索名称
                ...
                if (this.tabView == 'self') get_params.catalog_id = this.curTreeID;
                ...
                this.$api.crowd.getCrowds(get_params)
                    .then((data) => {
                        ...
                        data.items.forEach((item) => {
                            item.crowd_scale_filter = (item.crowd_scale || 0).toString().replace(/(\d)(?=(?:\d{3})+$)/g, '$1,');
                            item.estimated_scale_filter = (item.estimated_scale || 0).toString().replace(/(\d)(?=(?:\d{3})+$)/g, '$1,');
                        });
            ...
            sortChange(column) {
                ...
                this.sortName = column.order === 'normal' ? '' : column.key;
            ...
            tabViewFn(newV) {
                ...
                if (newV == 'node') {
                    ...
                    for (let i = 0; i < role_view.length; i++) {
                        if (role_view[i].id === 'catalog_group_crowd_in_node') {
        ...
        mounted() {
            this.$tools.bus.$on('updateCrowdList', () => {
                this.getList();
            });
        },
        beforeDestroy() {
            this.$tools.bus.$off('updateCrowdList');
        }
    };
</script>
